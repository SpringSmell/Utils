...
object MPUtils {
    ...
    private fun setPlayFile(context: Context,path: String): MediaPlayer = try {
        initMp()
        mp?.reset()
        if (HttpUtils.isHttpUrlFormRight(path)) mp?.setDataSource(context.applicationContext, Uri.parse(path)) else mp?.setDataSource(path)
        mp!!
    } catch (O_O: Exception) {
        mp!!
    }

    fun play(context: Context,path: String, onProgressListener: OnProgressListener): Boolean = try {
        MPUtils.onProgressListener = onProgressListener
        setPlayFile(context,path)
        lastAction = 1
        mp?.prepareAsync()
        true
    } catch (O_O: Exception) {
