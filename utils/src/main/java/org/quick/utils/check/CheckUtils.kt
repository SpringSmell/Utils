...
object CheckUtils {
    ...
    fun isHttpUrl(url: String?) =
        !isEmpty(url) && (url!!.toLowerCase().startsWith("http://") || url.toLowerCase().startsWith(
            "https://"
        ))
    ...
    fun isWifi(context: Context) = try {
        (context.applicationContext.getSystemService(Context.CONNECTIVITY_SERVICE) as ConnectivityManager).activeNetworkInfo.type == ConnectivityManager.TYPE_WIFI
    } catch (O_o: Exception) {
        false
    }
     ...
     * @return 有效：返回"" 无效：返回String信息
     */
    fun checkBankCard(context: Context,binNum: Long) = BankCardUtils.getNameOfBank(context,binNum)
